...
def main(cfg: DictConfig):
    ...
    gaussian_predictor = load_hmr_predictor(cfg)
    ...
    dataloader = DataLoader(dataset, 
                            ...
                            shuffle=True,
                            num_workers=cfg.opt.get("num_workers_train", 10))

    val_dataset = get_dataset(cfg, "val")
    val_dataloader = DataLoader(val_dataset, 
                                batch_size=1,
                                shuffle=False,
                                num_workers=cfg.opt.get("num_workers_val", 4),
    ...
    for iteration in range(first_iter, cfg.opt.iterations + 1):        
        ...
        gaussian_splats, _ = gaussian_predictor(
            input_images,
        ...
        if cfg.opt.get("offset_penalty", False):
            offsets = gaussian_splats["xyz_offsets"]
            norms = torch.linalg.norm(offsets, dim=-1)
            penalty = torch.mean(norms)
            total_loss += cfg.opt.get("offsets_coeff", 0.1) * penalty
        
        if cfg.opt.alpha_loss:
            alpha_loss = loss_fn_alpha(rendered_alphas.squeeze(), gt_masks.squeeze())
